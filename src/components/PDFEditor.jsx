...
export default function PDFEditor({ pdfBytes, signatureDataUrl, onConfirm }) {
  ...
  const [sigPos, setSigPos] = useState({ x: 40, y: 40, width: DEFAULT_SIG_W, height: DEFAULT_SIG_H });
  const [sigPage, setSigPage] = useState(0); // the page the user placed the sig on
  ...
  useEffect(() => {
    ...
    setSigPos((prev) => ({
      ...prev,
      x: Math.min(prev.x, canvasDims.width - prev.width),
      y: Math.min(prev.y, canvasDims.height - prev.height),
    }));
  }, [canvasDims]);
  ...
  const handleConfirm = () => {
    ...
    onConfirm({ placement: sigPos, canvasDims, pageIndex });
    addToast('Signature placed! Generating your signed PDF…', 'info');
  };
  ...
  return (
    <div className="animate-fadeup flex flex-col gap-5 w-full">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        {/* Size hint */}
        <div className="text-xs text-slate-500 glass-subtle px-3 py-1.5">
          ...
          Drag to move &bull; Handles to resize
        </div>
      ...
      <div ref={scrollRef} className="pdf-scroll-area glass" style={{ padding: 16, textAlign: 'center' }}>
        ...
        >
          ...
          {canvasDims.width > 0 && signatureDataUrl && (
            <Rnd
              ...
              minWidth={60}
              minHeight={24}
              maxWidth={canvasDims.width - sigPos.x}
              maxHeight={canvasDims.height - sigPos.y}
              enableResizing={{
